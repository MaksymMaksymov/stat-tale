...
function drawMap0_1(data, map) {
    ...
    canvas.width = tileSize * data.width * 3;
    canvas.height = tileSize * data.height * 3;
        ...
        for (let w = 0; w < data.width; w++) {
            if (typeof data.draw_info !== 'undefined') {
                ...
                for (let s = 0; s < cellSprites.length; s++) {
                    ...
                    for (let i = 0; i < 3; i++) {
                        for (let j = 0; j < 3; j++) {
                            ctx.drawImage(tmpCanvas, dx + i * tileSize, dy + j * tileSize, tileSize, tileSize);
                        }
                    }
...
(function() {
    ...
    sprite.src = "./images/map_alternative_2_new.png";
    sprite.mapType = map;
